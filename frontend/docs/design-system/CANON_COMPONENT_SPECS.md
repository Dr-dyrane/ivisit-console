...
| **Mobile sheet PRIMARY (neutral filled by default)** | `flex h-12 flex-[1.2] items-center justify-center gap-2 rounded-button text-sm font-bold tracking-wide transition-transform disabled:opacity-50` + inline neutral elevation `boxShadow: '0 6px 16px rgb(0 0 0 / 0.12)'`; default fill is `hsl(var(--foreground))` with `hsl(var(--background))` text. Pass an explicit literal/status tone only for a proved state action. Never default to red `--primary`. | MobileSheetActions:48-68 | `whileTap 0.96` |
