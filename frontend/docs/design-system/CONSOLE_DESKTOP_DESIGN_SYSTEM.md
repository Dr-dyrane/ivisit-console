...
| Select-all / bulk | Admin-only, header checkbox trigger + BulkActionBar (Requests); trigger regressed twice before pinning. Fail-closed pages (Visits) exclude selection entirely | Requests selection commits + Visits ledger |
