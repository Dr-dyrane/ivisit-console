...
| Select-all / bulk | Admin-only, header checkbox + BulkActionBar; the MECHANISM always renders on sortable-list surfaces, bulk WRITES stay per-page fail-closed (disabled with reason). Regressed a THIRD time at the Hospitals adoption (2026-07-09, user: "the table lacks the multiple select triggers") — per-page pins can't demand a mechanism a new page never decided on, so it is now an ESTATE LAW in ConsoleDesignSystem.contract: every SortableColumnHeader surface must carry `useRowSelection`/`handleToggleSelect` or the literal marker `selection excluded by decision: <ref>` | Requests selection commits + estate gate `dc899e1b` |
