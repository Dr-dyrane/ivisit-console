...
| Status-pill sizing | Status pills are content-sized labels, never grid-width bars. Shared `StatusPill` owns `w-fit`, `justify-self-start`, and no-wrap behavior so direct grid children match wrapped rows on Organizations, Subscribers, Pricing, and Insurance. | recurrence lock 2026-07-12 |
| Selection-required management registries | Organizations, Insurance, Email Subscribers, Pricing, and Payments may not satisfy the donor registry with a selection exclusion merely because their bulk mutation receiver is unproved. Keep checkbox toggle, shift-range, exact-visible-row select-all, indeterminate state, and `BulkActionBar`; keep unsupported actions disabled with their reason. | recurrence lock 2026-07-12 |
...
| **Interaction-completeness gate** (the polish backstop) | The Motion & Interaction Canon + UX-Completeness checklist were DOCS/manual — so Ambulances passed structure/mechanism/data gates and still shipped without a modal submit spinner and with un-pressable row buttons (the "relying on memory" failure). `ConsoleDesignSystem.contract` now enforces, per list-workspace page + its paired modal: `active:scale-*` (press) · NO `initial={{` (no stage-reveal) · `isFetching={isFetching}` (honest refetch) · `hasFilter ?` (branched empty) · `animate-spin` in the write surface (submit spinner). Composition-guaranteed items (SignalPanel no-entrance, KpiStrip spinner/aria-pressed, UpdatingPill role=status, ListRowShell layout=position) are NOT re-checked. Floor = presence, not per-control — per-button press stays a page pin. Proven to bite (would have flagged the pre-fix Ambulances modal). | interaction gate (this pass); canon `083c63f8`/`ff9ab49c`/`27dbb30b` |
...
| **Dock-FAB completeness + persona-pass** (mobile canon, shared by parity) | Two top-of-chain page-close rules the mobile lane owns and the desktop shares: (1) every FAB-suppressing list/dashboard route MUST own a dock FAB action (`getRouteOwnedMobileAction`) — data-sync-guided, RBAC-gated by `canReachRoute` — or an honest action-less exemption, else the dock collapses to a lone centered pill (Ambulances/Approvals/Users). Read-only blocks unproved writes, not a proved Stats/Review action; (2) a provider's avatar/label is its SUB-PERSONA from `provider_type` (responder/clinician), never a generic "Provider" — split only where the data carries it. Full rules → `MOBILE_DESIGN_SYSTEM.md` Decisions log 2026-07-10. | FAB-completeness guard (`check-mobile-grammar.js`) + `MobileUsers.jsx` persona helpers |
