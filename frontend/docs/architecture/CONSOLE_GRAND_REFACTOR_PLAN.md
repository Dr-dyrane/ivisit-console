...
| `APP-01` | Extract `AppRoutes`, `AppLayout`, and `AppShell` without provider/order changes | `src/App.js` plus new `src/app/**` modules and app-shell contracts | In progress - Erdos (`019f5caf-bae4-7bf3-b0e5-8da2c0950c0f`) | Provider order, lazy routes, protected-route metadata, shell chrome, startup overlay, and global modal mounts unchanged |
| `CTX-01` | Split `PageDataContext` compatibility facade by domain without changing its public value contract | `src/contexts/PageDataContext.jsx`, new domain context/query adapters, PageData contracts | In progress - Newton (`019f5caf-d13a-7fd2-a1dc-6d7dd5af8650`) | Consumer key inventory, route startup domains, query cancellation, exact loading/error semantics, no duplicate reads |
