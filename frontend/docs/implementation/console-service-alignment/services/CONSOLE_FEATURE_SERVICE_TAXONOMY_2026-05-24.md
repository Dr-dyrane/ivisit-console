...
| Console operator Copilot | Route-local explanation of already-authorized Console projections; proposal-only and separate from patient triage. P0 covers Today, organization readiness and emergency next-action evidence. | Local deterministic adapter under `frontend/src/features/copilot`; no Supabase, RPC, Edge, model, SQL or write owner. Any later command must enter the owning feature pass and prove its receiver, authorization, idempotency and reflection contract first. | Cross-cutting presentation boundary |
