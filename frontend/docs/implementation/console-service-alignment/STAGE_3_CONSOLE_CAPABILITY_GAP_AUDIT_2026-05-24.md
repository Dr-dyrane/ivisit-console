...
| Auth and security exceptions | `src/contexts/AuthContext.jsx:171-277`, `src/components/pages/LoginPage.jsx:73-195`, `src/components/pages/SetPasswordPage.jsx:25-55`, `src/components/modals/SecurityModal.jsx:29-87` | Auth, MFA, reset, and security flows necessarily call Supabase Auth directly in UI-adjacent code. | accepted exception, needs wrapper decision | Keep direct Auth where the Supabase SDK contract is the product boundary, but document which calls belong in `authService` versus route-local forms. |
...
| `HospitalsPage` | Uses `usePageData()` and `hospitalsService` (`HospitalsPage.jsx:9`, `HospitalsPage.jsx:33`). | Page scan did not show direct Supabase reads; modal/service paths still own provider discovery and capacity drift. | partial alignment | Keep CRUD in `hospitalsService`, but move capacity and discovery receiver choices into explicit service methods. |
...
| `SecurityModal` | Direct MFA SDK calls (`SecurityModal.jsx:29-87`). | Accepted SDK-adjacent exception, but auth UI owns error/loading semantics. | Decide whether MFA belongs in `authService` or remains a documented route/modal-local Supabase Auth boundary. |
