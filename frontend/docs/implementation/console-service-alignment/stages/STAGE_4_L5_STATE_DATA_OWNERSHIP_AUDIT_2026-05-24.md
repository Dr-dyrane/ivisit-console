...
Hospitals, pricing, visits, wallet, verification, subscribers, and analytics can render rows/KPIs, but they cannot yet consume the full source-of-truth contract needed for safe operations.
...
| --- | --- | --- | --- | --- |
| Emergency request lifecycle | Emergency requests, map/God mode, visits linkage | Move page/context reads behind an emergency query owner; centralize emergency list/count/search; keep modal/detail realtime scoped; replace generic payment-event refetch with domain invalidation. | Repair fallback create parity, complete/dispatch lifecycle legality, tracking-ready route/ETA consumption, and emergency-to-visit creation contract. | Emergency is the highest user-safety surface and drives visits, map, payments, and dispatch. |
...
| Hospitals, availability, discovery, and pricing | Hospitals/capacity, pricing, dashboard summaries | Centralize hospital/pricing reads; expose hospital-scoped pricing honestly; route ER wait/capacity UI through one service owner; make discovery fallback explicitly read-only. | Decide canonical availability RPC/direct-edit boundary; fix provider taxonomy/dispatch eligibility fields; repair public discovery write authority; solve multi-hospital org pricing semantics. | Hospital and pricing data are consumed by app checkout and dispatch routing; wrong scope silently affects many facilities. |
| Provider operations and telemetry | Ambulances, responder telemetry, doctors, scheduling, map projection | Move ambulance/doctor counts, lookups, and modal support reads behind services; split map telemetry projection from canonical CRUD; consolidate schedule read model. | Replace unsafe direct operational ambulance status/location writes; define driver/profile assignment mirror; decide doctor profile automation and `doctor_schedules` ownership. | Dispatch quality depends on accurate responder and provider state, but some work can be service-boundary cleanup before backend repair. |
...
| Content, support, subscribers, and email | Health news/content, support tickets, subscribers/email, notifications | Move health-news KPIs to service; keep support tickets on hook pattern; consolidate subscriber services; expose fallback/degraded states visibly. | Fix subscriber lifecycle owner, schema-current payloads, welcome/unsubscribe/campaign state, support RLS/status taxonomy, content authoring policy. | These are less urgent than emergency/payment but visible to patients and operators; subscriber email can duplicate external sends. |
