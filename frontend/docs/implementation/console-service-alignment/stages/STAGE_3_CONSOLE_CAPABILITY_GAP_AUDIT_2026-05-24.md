...
| `SubscriptionManagementPage` and mounted shell consumers | Route uses `useSubscription`; context/navigation, analytics and home surfaces also consume the hook (`SubscriptionManagementPage.jsx:43`; `ContextPanel.jsx:56`; `Analytics.jsx:83`; `BentoHome.jsx:444`). | Hook instances perform full reads and broad realtime; modal/navigation surfaces invoke email commands separately. | mixed ownership | Keep subscriber lifecycle and email sends in one subscription facade, consolidate mounted reads/channels, then align Edge Function ownership. |
...
| `subscribers` | `subscriptionService.subscribeToSubscribers()` and `subscribeToNewSubscribers()` (`subscriptionService.js:362-391`), duplicate `subscribersService.subscribeToSubscribers()` (`subscribersService.js:204-222`), and repeated `useSubscription` mounts (`useSubscription.js:147-202`). | Subscriber lifecycle has duplicate service families and repeated mounted reads/channels even though the previously charted page-level sender is no longer present. | One subscriber/subscription service facade and one bounded projection/invalidation owner. |
...
| Subscription write authority after payload repair | Current `subscriptionService` uses a fixed allowed-field payload (`subscriptionService.js:11-49`) but still exposes update/delete/status/type operations under public-insert/admin-read RLS. | high risk | Preserve the payload repair and remove or gate unsupported subscriber management commands behind an authorized receiver. |
...
4. The subscriber payload has been repaired to current schema fields; this does not authorize the remaining browser update/delete/status commands or close welcome-email lifecycle ownership.
...
| Subscriptions actions | Table skeleton and subscriber create/update/delete toasts exist; email action modal reports aggregate bulk outcomes and unmarked welcome delivery (`SubscriptionManagementPage.jsx:203-256`; `SubscriptionModal.jsx:107-164`). | feedback present, contract risky | Unsupported update/delete commands and the `sendWelcome` versus `process-subscribers` durable-state split remain the larger risk. |
