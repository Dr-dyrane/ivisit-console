...
| --- | --- | --- | --- | --- |
| Pass 1 Emergency lifecycle, communication, clinical handoff and cash/payment truth | Strongest coverage: evidence audit, route ledger, projection target, modal raw-field matrix, action contract, checklist and app-consequence chain; 2026-07-14 adversarial App/Console/backend gate added. | Partial. Detail and cash honesty improved, but card release ownership, role-specific lifecycle, responder decline/requeue, exactly-one visit consequence, patient arrival semantics, chat and clinician handoff remain open. | Started. Current branch improves map/mobile action honesty without changing backend authority. | `NO-GO` for live dispatch until the Pass 1 P0 ledger passes against deployed backend truth. Onboarding and labelled demo testing may continue. |
...
| Pass 5 Provider operations and scheduling | Subplan, provider contract chart, service coverage, checklist and 2026-07-14 adversarial readiness decision exist. | Partial. Dispatcher legality, responder identity, staffing RLS, assignment readiness, telemetry, decline/requeue, standalone ambulance-service scope and schedule/capacity boundaries are explicit. | Frontend slice in progress: dispatcher admission, responder-first client scope, honest driver assignment controls, foreground telemetry, 5 km render lens and compact tablet composition. No database change. | Preserve the frontend corrections, keep staffing/schedule/decline unavailable, and treat responder-owned RLS, atomic staffing/assignment, background telemetry/push and role-specific lifecycle as live-launch blockers. |
