...
## Pass 2A Surface-By-Surface Confirmation Ledger

This ledger is the next executable audit checklist for wallet and finance. It is intentionally read-only and disposition-focused. Runtime implementation begins only after each row is confirmed against current source and either retained, disabled, moved to the finance projection owner, or blocked by receiver/RLS proof.
...
| --- | --- | --- | --- | --- | --- |
| `/wallet` desktop route reads | `WalletManagementPage.jsx:63-131` directly reads platform/org wallet, latest `50` ledger rows, latest `50` payments, profile enrichment and payment methods. | Route tabs and finance workspace layout can remain. | Wallet row, ledger preview, payment preview, profile summary, payment methods, account status and degraded states move to `WalletFinanceProjection`. | Do not add new route-local finance reads or N+1 enrichment. | Projection returns actor scope, wallet display, ledger preview, payment preview, method state and load/degraded states. |
| `/wallet` route maintenance repair | `WalletManagementPage` imports `backfillMissingFeeLedger`; Pass 2 evidence shows route-mount repair behavior. | None in normal product UI. | Maintenance readiness can exist only as a separate explicit operator-maintenance projection. | Remove/disable ordinary route access to fee-ledger repair/backfill. | Normal wallet render performs no ledger/payment mutation. |
| `/wallet` ledger footer/export | Route loads `.limit(50)` and exports loaded ledger rows. | Recent preview display can remain if labelled. | Export readiness, row count basis, window label and dataset scope move to projection. | Disable full-history export claims; do not call preview export complete ledger evidence. | UI states `Latest 50` or server-backed scope; export button disabled unless scope is truthful. |
| `/wallet` payment history | Route loads recent payments and enriches users per row. | Payment preview list can remain. | Payment row display id, payer summary, amount/currency/status/source and window label move to projection. | Do not infer revenue or success metrics from preview rows without basis label. | Projection distinguishes pending, completed, failed, refunded and unavailable payment states. |
| `/wallet` analytics modal | `WalletManagementPage.jsx:716-733` opens generic `AnalyticsModal` from capped ledger/payment/method arrays. | Analytics affordance can remain as unavailable or projection-backed. | Finance analytics basis and source labels move to projection or Pass 8 analytics owner. | Disable generic analytics if it would render capped previews as complete finance evidence. | Modal consumes `metrics.basis`, `windowLabel`, `scope`, and unavailable states. |
| `MobileWallet` | `MobileWallet.jsx:64-118,216-258` computes credit count, payment success and `LIVE` labels from preview arrays. | Mobile wallet composition can remain. | Mobile KPIs, trends, counts and actions move to projection slices. | Remove `LIVE`/complete-analytics implication from preview-derived metrics. | Mobile receives projection-derived metric basis and command readiness. |
| `WalletPanel` | `WalletPanel.jsx:19-143` renders balance, static trust labels, four ledger rows and `projection || wallet.balance * 0.12`. | Read-only wallet summary panel can remain. | Balance, projected yield, ledger preview, linked/verified labels and export readiness move to projection. | Remove balance-derived yield fallback and static `Verified`/`Linked` copy unless receiver-backed. | Panel consumes context projection slice, not raw `walletData`. |
| `PageDataContext.walletData` | `PageDataContext.jsx:548-563` performs separate wallet plus latest `10` ledger read. | Minimal shell slice can remain after owner exists. | Context wallet reads move to projection summary slice. | Do not let context become a second finance owner. | Context only composes projection output; it does not query wallet tables directly. |
| `BentoHome` and dashboard finance widgets | Pass 2 evidence records independent `getWalletSummary()` use. | Dashboard summary can remain if scoped. | Balance, today income, trend and basis move to projection or Pass 8 shell summary. | Disable sponsor/performance finance claims from unsupported preview values. | Dashboard displays source, scope, basis and unavailable state. |
| `Analytics` finance usage | Pass 2 evidence records independent `getFinanceAnalytics()` use. | Finance charts can remain only if source-labelled. | Finance metrics move to projection/server aggregate or Pass 8 analytics owner. | Do not render ledger-preview totals as complete analytics. | Chart basis is `server_analytics`, `ledger_preview`, or unavailable. |
| Top-up modal | `GlobalFinancialModals` invokes `topUpWallet()` and says wallet topped up after PaymentIntent creation. | Amount entry can remain as a pending payment attempt surface. | Top-up readiness, billing-method state, payment-intent state, Stripe confirmation state and wallet-credit reflection move to projection/command model. | Remove completed-funding copy; disable top-up success if wallet-credit receiver is unproved. | Top-up copy says pending/intent-created until payment, ledger and wallet reflection are proved. |
| Withdrawal/payout modal | `GlobalFinancialModals` checks display balance and says funds transfer instantly after `withdrawFunds()`. | Withdrawal form can remain unavailable or request-only. | Payout readiness, sufficiency/reservation, pending/paid/failed state and reflected ledger/balance state move to projection/command model. | Disable production payout action until backend reservation and failure reflection are proved. | No copy says instant, paid or deducted from display balance alone. |
| Billing/payment-method modal | `GlobalFinancialModals` lists methods, can create setup intent and labels every method `Primary`; imported `setPayoutMethod` is unused. | Setup-intent/card-linking surface can remain if scope is proven. | Method scope, remove readiness, primary payout method and setup result move to billing projection. | Remove static `Primary` badge and any remove action whose scope differs from the listed collection. | Method command uses the same platform/org scope for list, remove and selection. |
| Payment-method route removal | `WalletManagementPage` can list platform methods with `null` but delete with `profile.organization_id`. | Removal can remain only after scope proof. | Remove command state moves to method projection. | Disable removal when scope is missing, mixed or unknown. | The projected method row carries exact command scope and refreshed result target. |
| Emergency cash settlement dependency | Pass 1 route may call `walletService.processCashPayment()` with hospital-id fallback. | Cash approval display remains Pass 1; finance settlement remains Pass 2. | Manual settlement, eligibility, fee estimate and ledger reflection move to finance/emergency shared command contract. | Disable/defer post-completion cash settlement in emergency route until receiver is proved. | No emergency or wallet copy says fee deducted without reflected ledger/payment truth. |
| Pricing route top-up entry | Shared context action can open `openTopUpModal` from `/pricing`. | Cross-route finance handoff can remain only if deliberate. | Handoff readiness moves to finance command readiness plus Pass 3 pricing action ownership. | Remove accidental pricing primary-action funding entry while pricing management is being audited. | `/pricing` actions cannot silently become wallet funding commands. |
| Organization wallet side reads | `organizationsService.getOrganizations()` maps `organization_wallets` into `wallet_balance`. | Admin organization balance preview can remain after scope label. | Organization balance preview moves to finance/org projection or Pass 4 org projection. | Do not treat mapped wallet balance as payout-ready or ledger-complete. | Org table labels balance scope and unavailable state. |

Pass 2A stop condition: if a finance surface cannot consume the shared projection yet, it must be marked unavailable or left untouched. Do not patch that surface with another balance fallback, preview-derived metric, unscoped Stripe call, repair helper, or local export.
