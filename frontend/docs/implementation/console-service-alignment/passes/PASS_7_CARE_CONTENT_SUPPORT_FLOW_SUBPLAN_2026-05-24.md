...
- `frontend/src/components/views/HealthNewsListView.jsx`
- `frontend/src/components/views/HealthNewsTableView.jsx`
...
- `frontend/src/components/views/SupportTicketSimpleListView.jsx`
...
- `HealthNewsManagementPage` gates publish/edit/delete on grid cards with `isAdmin() || isOrgAdmin() || isSponsor()`, but passes the callable `isAdmin` function into `HealthNewsListView` and `HealthNewsTableView`; both child renderers use `if (isAdmin)` truthiness. List/table modes therefore expose publish/edit/delete controls independently of the evaluated grid role check and before content write authority is proven.
...
- `SupportTicketSimpleListView` is source-present with the same callable-`isAdmin` truthiness pattern and corrupted visible separators, but no mounted importer was found in the current source scan. It is a dormant unsafe renderer, not an additional live support capability; any future mount must consume the same support projection/capability and encoding gate.
...
| Health-news desktop view-mode commands | Grid checks three management roles, while list/table treat an `isAdmin` function reference as granted authority and expose publish/edit/delete. | Resolve published-feed command capability once before any renderer receives handlers; keep authoring commands unavailable until receiver/policy proof exists. |
...
| Dormant simple support list renderer | Yes: source-present ticket row/command renderer with corrupted separators; no mounted importer found. | Yes: it would expose edit/delete on callable-role truthiness if mounted. | Keep unmounted or migrate with the active support variant contract before any future use. | Audited dormant unsafe renderer. |
...
| Health-news desktop list/table variants | Yes: same published rows displayed under selectable layouts. | Yes: publish/edit/delete are exposed through function-object truthiness rather than evaluated capability. | Do not compose authoring callbacks until published-feed write authority and actor capability are proved. | Audited variant-authority blocker. |
...
| Health-news variant command leakage | `frontend/src/components/pages/HealthNewsManagementPage.jsx:825-886` evaluates roles in grid but passes `isAdmin={isAdmin}` into list/table; `frontend/src/components/views/HealthNewsListView.jsx:67-91` and `HealthNewsTableView.jsx:107-121` test the function reference as a boolean. | Switching to list/table can reveal publish/edit/delete regardless of the evaluated role. | Project `canPublishContent`/`canEditContent`/`canDeleteContent` before composing variants; default unavailable while content write policy is unproved. |
