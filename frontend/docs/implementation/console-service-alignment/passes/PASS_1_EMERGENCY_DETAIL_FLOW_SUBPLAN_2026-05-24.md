...
- `C:/Users/Dyrane/Documents/GitHub/ivisit-app/services/serviceCostService.js`
- `C:/Users/Dyrane/Documents/GitHub/ivisit-app/services/notificationDispatcher.js`
- `C:/Users/Dyrane/Documents/GitHub/ivisit-app/services/triageCopilotService.js`
...
- `emergencyResponseService.dispatchEmergency()` obtains unbounded available ambulance candidates, unbounded verified hospital candidates when no hospital is selected, and unbounded matching doctor candidates, then chooses the first available/suitable result. Its `findBestHospital(patientLocation, specialty)` helper does not use `patientLocation`, so automatic dispatch can assign a facility from query order rather than proved proximity and capacity readiness.
...
- The patient app calculates emergency cost through `calculate_emergency_cost_v2`, while `serviceCostService` can return a mock cost after RPC failure; Console emergency payment labels therefore need canonical/degraded quote provenance rather than assuming a rate row is the patient charge.
- Patient cash requests can notify organization admins through `notify_cash_approval_org_admins`, with an app-side fallback insertion path; a Console notification or approval badge is delivery/reflection evidence, not proof of payment settlement or dispatch release.
- `triage-copilot` and `demo-dispatch-reply`/`demo-approve-cash-payment` are app-origin branches: triage may contribute persisted authorized request context, while demo replies/approval must remain segregated from production emergency truth.
...
| Dispatch resource selection | Dispatch service performs unbounded candidate reads and first-result ambulance/hospital/doctor selection; its named hospital proximity input is unused. | Command-owned assignment projection consuming bounded/scoped responder, facility-capacity/eligibility and clinician-readiness candidates from Passes 3 and 5 before lifecycle mutation. |
...
| Patient quote provenance | Patient emergency cost RPC can fail into a mock-cost app fallback; Console does not currently represent this source state. | Emergency payment/price projection declares canonical, degraded/fallback or unavailable quote origin before presenting payable outcome. |
| Cash approval notification delivery | App notification dispatcher may call RPC or fall back to client-generated notification inserts. | Notification/reflection owner shared with Pass 2/8; do not treat delivery as approval, settlement or dispatch evidence. |
| Triage and demo branches | Patient triage suggestion and demo reply/approval functions can influence app-visible flow but are not production Console commands. | Render approved persisted triage context only when scoped; explicitly exclude demo writers from production operation claims. |
...
| Dispatch candidate resolution behind page/map actions | No automatic-selection detail is rendered before the command; selection occurs inside `emergencyResponseService`. | Loads full available ambulance/verified hospital/matching-doctor collections and picks first suitable candidate before `console_dispatch_emergency`. | **Blocked command selection defect.** Operators cannot verify why a responder/facility/clinician was assigned; command needs scoped readiness/proximity/capacity projection and refreshed assigned-resource truth. |
...
| Quote, triage and notification provenance | Patient services use `calculate_emergency_cost_v2`, optional `triage-copilot`, and `notify_cash_approval_org_admins` with failure/fallback behavior. | Detail/read projections identify persisted operational truth and degraded/unavailable origin; no mock quote, AI suggestion or notification receipt is presented as confirmed care/payment lifecycle state. |
...
| `emergencyResponseService` candidate reads | `emergencyResponseService.js:31-61,141-163` calls unbounded `getAvailableAmbulances()`, `getHospitals({ verified: true })` and `getDoctors(...)`, selects the first candidate, and ignores `patientLocation` during hospital selection. | Blocked. Resource choice is neither bounded nor proximity/readiness-authoritative before the dispatch RPC. | Make resource-candidate selection an explicit command-facade dependency on Pass 3/5 projections; render selected-resource basis or unavailable state before final dispatch success. |
...
| Select dispatch resources | `emergencyResponseService.js:31-61,141-163` chooses the first available ambulance, first suitable verified hospital and first matching doctor from unbounded candidate reads; the hospital helper ignores patient location. | Pass 3 facility capacity/eligibility projection plus Pass 5 responder/clinician readiness projection, consumed by the Pass 1 command facade. | Candidate result must state actor scope, bounds, selection basis, facility/responder/clinician ids, capacity/availability evidence and degraded/unavailable reason before dispatch is presented as assignable. | Do not label first-row selection as nearest, optimal or safe automated assignment. |
...
| Patient quote and cash notification receivers | App `serviceCostService.js` calls `calculate_emergency_cost_v2` and may substitute mock cost on failure; app `notificationDispatcher.js` calls `notify_cash_approval_org_admins` then can fall back to notification inserts. | Detail surfaces can otherwise show a charge or notification as stronger truth than its receiver proves. | Extend payment/action projection with quote provenance and notification-delivery status only where authorized; settlement/release still requires refreshed payment/request/ledger truth. |
| Patient-only and demo receiver separation | App `triageCopilotService.js` invokes `triage-copilot`; app payment/chat branches invoke `demo-approve-cash-payment` and `demo-dispatch-reply`. | Console implementation could incorrectly promise AI triage or contaminate live approval/chat evidence with demo outcomes. | Treat persisted triage as an optional authorized input projection only; keep demo writers excluded or explicitly labelled in any test/demo surface. |
...
## Pass 1A Route-By-Route Confirmation Ledger

This ledger is the next executable audit checklist. It converts the broad Pass 1 contracts into the exact disposition required for each currently mounted emergency surface. Runtime implementation begins only after each row is confirmed against current source and either retained, disabled, moved to an owner, or blocked with a named receiver/RLS reason.

| Mounted surface | Current source truth | Fields or controls to retain | Fields or controls to move to owner | Fields or controls to disable/remove first | First implementation gate |
| --- | --- | --- | --- | --- | --- |
| `/emergencies` route fetch | `EmergencyRequestsPage.jsx:119-217` owns count, filters, row query, payment enrichment and selected-row refresh. | Existing page, filter and pagination intent can remain. | Move count, row query, payment enrichment, selected-row refresh and degraded states into `getEmergencyListProjection()` or equivalent. | Do not add new list fields or actions from page-local rows. | Projection must return row window, total count, payment visibility, partial failures, selected-row state and refresh targets. |
| `/emergencies` route realtime | `EmergencyRequestsPage.jsx:219-229` subscribes broadly to `emergency_requests` and `payments`. | Route-level invalidation can remain conceptually. | Move channel ownership into list projection or a route controller that distinguishes list invalidation from detail invalidation. | Do not let page payment realtime override scoped detail payment truth. | Channel plan names table, filter/scope, refresh target and cleanup. |
| Emergency grid cards | `EmergencyRequestsPage.jsx:960-1134` renders raw patient, contact, location, hospital, status, payment and action fields. | Card layout may remain. | Patient, service, status, location, facility, payment, responder, clinical and action state move to list-row projection. | Disable actions whose projection state cannot prove receiver eligibility. | Grid renders only projection fields or explicit unavailable/degraded states. |
| `EmergencyRequestListView` | `EmergencyRequestListView.jsx:43-230` renders aliases, `LocationCell`, local action state and local clinical lookup. | List layout and route navigation affordance can remain. | Clinical lookup, action state, location display and field aliases move to row projection. | Remove clinical record payload logs and local `getVisitByRequestId()` call from the leaf once row projection provides receiver. | Leaf receives row projection plus callbacks; it performs no service reads. |
| `EmergencyRequestTableView` | `EmergencyRequestTableView.jsx:93-260` repeats row field and clinical lookup logic. | Table layout and sorting affordance can remain. | Clinical receiver, payment/status labels, location display and action availability move to row projection. | Remove browser logging of fetched visit data before or during implementation. | Table consumes `clinicalOutcome.receiver` and does not emit unmounted custom events. |
| `MobileEmergency` | `MobileEmergency.jsx:106-147,344-465` derives KPI totals from rows, renders legacy aliases and labels a dispatch path as `Navigate`. | Mobile layout and stable-list behavior can remain. | KPI basis, row fields, status grouping and action labels move to projection. | Remove or relabel `Navigate` if it dispatches; remove `LIVE` claims for row-window-derived metrics. | Mobile receives the same row/action projection as desktop, with mobile-only composition. |
| `EmergencyDetailsModal` | `EmergencyDetailsModal.jsx:86-235` uses `getEmergencyDetailProjection()` and scoped subscription; JSX still renders many raw request fields. | Preserve detail projection, approve/decline refresh and scoped subscription. | Raw patient/service/status/location/payment/responder/clinical/report fields move behind detail projection keys. | Disable/remove inert `Call Patient`, `Generate Incident Report` and `openVisitModal` custom-event CTA until receivers are chosen. | Detail modal treats raw `request` as seed only; rendered fields come from projection or unavailable state. |
| Detail clinical CTA | `EmergencyDetailsModal.jsx:468-471` dispatches `openVisitModal`; `/visits?view=<id>` is the existing mounted route receiver. | Full clinical record navigation can remain if it uses route navigation. | Receiver choice moves to `clinicalOutcome.receiver`. | Do not fire `openVisitModal` from `/emergencies`; it has no valid mounted receiver there. | Choose `navigate_to_visits` or emergency-route read-only outcome panel. |
| Detail cash approval/decline | `EmergencyDetailsModal.jsx:121-153`; `emergencyService.js:559-608` uses `approve_cash_payment` / `decline_cash_payment`. | Preserve as canonical pending-cash approval lane. | Availability, amount, disabled reason, pending key and refreshed result copy move into `paymentDisplay` / `actionState`. | Remove data-bearing browser logs from cash approval/decline service path. | Success copy waits for refreshed request/payment projection. |
| Route dispatch/complete/retry | `EmergencyRequestsPage.jsx:417-577` directly checks cash eligibility, dispatches, completes, prompts manual cash, and retries payment. | Dispatch, complete and retry remain Pass 1 command concepts. | Command legality, canonical org/facility identity, pending state, post-command refresh and copy move to shared command facade. | Disable/defer manual cash settlement prompt unless Pass 2 finance receiver is proven. | No success copy says dispatched, completed, cash settled or fee deducted until refreshed projection proves it. |
| Desktop map marker detail | `MarkerDetailPanel.jsx:127-168` directly dispatches and completes from selected marker data. | Map marker visibility can remain as a consumer. | Marker fields and actions move to emergency row/action projection or a map-scoped projection. | Disable map dispatch/complete if it cannot consume the same command facade as the route. | Map cannot bypass cash, payment, responder or lifecycle legality. |
| Mobile map marker detail | `MobileMap.jsx:273-303` directly dispatches and completes from selected marker data. | Mobile map selected-marker composition can remain. | Selected-marker action state moves to shared emergency command model. | Disable mobile map dispatch/complete if projection is absent. | Mobile map action copy matches route command copy and refresh rules. |
| `EmergencyPanel` context panel | `EmergencyPanel.jsx:17-23,107-154` renders global summaries and emits modal/filter/analytics events. | Read-only summary panel can remain. | Recent request rows and KPI summaries move to bounded shell/emergency summary projection. | Do not let panel own create/filter/report truth or render mojibake copy. | Panel is read-only or uses mounted route receivers with explicit availability. |
| `LocationCell` emergency usage | `LocationCell.jsx:8-103` decodes mixed coordinate sources and calls Google geocoding. | Leaf address display can remain if it becomes render-only. | Coordinate normalization, source labels, external lookup state and disclosure policy move to emergency location projection. | Do not let geocoding failure look like missing backend location truth. | Projection distinguishes address, coordinate, geocode unavailable and external-map handoff states. |
