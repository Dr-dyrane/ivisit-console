...
| `SecurityModal` Auth SDK actions | MFA/password operations call Supabase Auth directly. | Own-user Auth adapter exception can remain. | Assurance level, secret display, challenge, verification and recovery state move to documented own-user security controller. | Do not treat direct Auth SDK calls as proof for admin user management. | Security modal verifies failure and recovery states without leaking secrets or profile data. |
