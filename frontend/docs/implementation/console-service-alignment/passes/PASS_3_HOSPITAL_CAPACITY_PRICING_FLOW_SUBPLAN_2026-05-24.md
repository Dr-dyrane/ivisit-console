...
- `frontend/src/components/views/PricingListView.jsx`
...
- `ivisit-app/services/serviceCostService.js`
...
- `HospitalsPage` intends a 20-row service-backed window, but calls `getHospitals({ limit: pagination.pageSize, offset: pagination.paginationRange.start, count: true })` while `usePagination()` exposes `itemsPerPage`, not `pageSize`. The resulting undefined `limit` and first-page zero `offset` do not trigger either limit/range branch in `hospitalsService.getHospitals()`, so the visible first-page list itself requests the unbounded authorized hospital collection and can render the backend response ceiling as one page.
- The same route also consumes `PageDataContext` stats derived from a second unbounded `getHospitals()` result; at the backend response ceiling this can display `1000` as total network/capacity truth independent of the visible row defect.
...
- `AppLayout` mounts `MapProvider` and `PageDataProvider` around all routes. `MapProvider` initializes `supabaseMapService.fetchInitialMapData()` before route authorization and that service loads hospitals without a limit; after authentication `PageDataContext.fetchHospitalsData()` separately loads hospitals without a limit. Opening `/hospitals` can therefore issue three independently unbounded hospital reads before pricing dependencies are considered.
- `MobileHospitals` mounts `PullToRefresh`, `useStableList` and `useLoadMoreControl`. Those controls preserve/display the defective page rows and request the next page through the same route owner; they do not repair missing server bounds or prove that a retained row buffer is current truth.
...
- Emergency dispatch also consumes facility truth: `emergencyResponseService.findBestHospital(patientLocation, specialty)` loads every verified hospital through unbounded `getHospitals({ verified: true })`, filters on available beds/specialty, ignores its patient-location argument and returns the first candidate. Facility readiness/proximity cannot be accepted merely because the dispatch RPC later persists a chosen hospital id.
...
- App `pricingService` and `serviceCostService` invoke `calculate_emergency_cost_v2`, and `serviceCostService` can substitute mock cost when the RPC fails. A Console price row or updated facility rate is therefore not, by itself, proof of the patient-visible emergency charge or quote quality.
...
| --- | --- | --- |
| Hospital list/stats | Page passes nonexistent `pagination.pageSize` to `getHospitals()`, making its first-page list query unbounded; global `PageDataContext` independently calls unbounded `getHospitals()` and derives network/capacity totals from a potentially capped collection; page filter/KPI/sort state is not authoritative for rendered rows. | Facility read owner with a proved page-size/range/filter/sort/count query plus scoped aggregate/count projections. |
...
| Dispatch facility candidate selection | Emergency dispatch loads unbounded verified facilities, tests local bed/specialty fields and selects the first row without using patient location. | Bounded/scoped facility eligibility and capacity candidate projection with explicit proximity/selection basis, consumed by the Pass 1 dispatch command facade. |
...
| Patient emergency cost provenance | App cost calculation can use `calculate_emergency_cost_v2` or a mock fallback after RPC failure, while Console price management exposes raw facility/global rows. | Quote-provenance dependency: price management renders scope and mutation consequence accurately, and any patient-charge comparison declares canonical/degraded/unavailable quote source. |
...
| --- | --- | --- | --- | --- |
| `/hospitals` grid/list/table/mobile; route permits `org_admin` and above while the context panel advertises admin/org-admin management. | Facility identity, address/image, verification/status, beds/ICU, fleet, ER wait and rating; table/page count. Mobile also renders operational KPI and trend panels. | The list supplies undefined `pagination.pageSize` to a service that limits only for truthy `limit`, so first-page rows are unbounded. Page filters/KPI selection/table sort do not govern retrieval/rendered rows; mobile retains/reduces those rows for total/available beds/fleet and labels the result `LIVE`. | View, edit, delete, schedule, refresh and load-more controls are rendered across variants. | Read remains open until one bounded facility projection governs desktop/mobile pagination, refresh, buffered loading and metrics; edit/delete/schedule require field-by-field command/RLS proof before retained. |
...
| Emergency cost calculation provenance. | The patient app calls `calculate_emergency_cost_v2`; one app service can substitute mock cost after calculation failure. | Console-managed rates affect the input landscape but do not alone establish the displayed/charged patient result. | Any parity validation must distinguish canonical RPC quote, degraded/fallback quote and unavailable comparison rather than declaring aligned pricing from table CRUD alone. |
...
| --- | --- | --- |
| Hospital desktop variant command drift | `frontend/src/components/pages/HospitalsPage.jsx:719-795` gates grid edit/delete to admin/org-admin but passes edit/delete to list/table; `frontend/src/components/views/HospitalListView.jsx:89-124` renders the supplied controls without a role gate, while `HospitalTableView.jsx:32,180-195` includes provider in `canManage`. | Facility edit/delete authority changes with view mode. One facility row capability projection must govern grid/list/table/mobile before retaining mutation controls. |
| Hospitals route intended page row read | `frontend/src/components/pages/HospitalsPage.jsx:62,70-102` creates `usePagination(20)` but sends missing `pagination.pageSize` into `getHospitals({ limit, offset, count })`; `frontend/src/hooks/usePagination.js:8-67` exports `itemsPerPage` only. | The current first-page query is unbounded, not paged. Replace it through the facility read owner with an explicit bounded page/filter/sort/count contract before rendering rows or totals as operational truth. |
...
| Hospital read service paging | `hospitalsService.js:177-217` applies `.limit(...)` only for truthy `filter.limit` and `.range(...)` only for truthy `filter.offset`; zero-offset first-page requests with an undefined limit remain unbounded. It also supports only status, verified, verification status and specialty filters. | Page size, zero-offset behavior, search, KPI and sort state need an explicit query contract before implementation. |
...
| 3 | `/hospitals` unbounded-row repair and read migration | After slice 1 | Replace the `pageSize`/`itemsPerPage` mismatch and route desktop, table/list/grid, mobile refresh/load-more and panel facility displays through one projection with real page/filter/sort/count semantics. | Do not retain an unbounded first page, compute network/capacity totals or label mobile buffered metrics `LIVE` from page rows or unbounded bootstrap lists. |
...
- `/hospitals` first-page rows are proved bounded; no missing page-size field can silently disable its limit/range contract.
...
| --- | --- | --- | --- | --- | --- |
| `/hospitals` route rows | `HospitalsPage.jsx:62,70-102` intends paging but passes absent `pagination.pageSize`; `getHospitals()` does not limit the zero-offset first request without a truthy limit. | Route workspace, pagination intent and row viewing can remain. | Bounded page size/range, filters, KPI selection, sort, row projection, count and degraded state move to facility projection. | Do not treat the present unbounded first-page rows as a valid page, network total or capacity aggregate. | Projection proves bounded page rows, total/count basis, filter/sort basis and degraded reasons. |
