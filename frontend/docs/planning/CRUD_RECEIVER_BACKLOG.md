...
The mobile dock FAB surfaces the most useful receiver-backed action admitted for each route. It does
not promote a fail-closed writer merely because dormant create chrome exists. Subscriptions,
Organizations, Pricing, and read-only Health News therefore open their proved statistics surfaces;
when an authoring receiver is admitted, its page handler, FAB, gate doc, and contract must change
together (see `MOBILE_DESIGN_SYSTEM.md` FAB canon).
