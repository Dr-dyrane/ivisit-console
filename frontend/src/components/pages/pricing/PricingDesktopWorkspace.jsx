...
const count = (summary, total, id) => id === 'all' ? Number(total || 0) : Number(summary?.[OPTIONS.find((option) => option.id === id)?.countKey] || 0);
...
const average = (summary) => Number.isFinite(Number(summary?.averageAmount))
  ? new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(Number(summary.averageAmount))
  : null;

export const PricingDesktopWorkspace = ({ rows, summary, totalCount, loading, isFetching, error, filters, setFilters, retry, pagination, sortConfig, onSort, focusedPrice, setFocused, moduleRailItems, routingPath, onRailNavigate }) => {
  ...
  const averageAmount = average(summary);
  const signal = failedEmpty ? { icon: AlertTriangle, tone: 'danger', label: 'Load failed', headline: 'Pricing did not load', subhead: 'Retry to load the pricing projection.' } : { icon: option.icon, tone: activeCount ? (active === 'override' ? 'clear' : active === 'global' ? 'warning' : 'primary') : 'muted', label: option.label, headline: activeCount ? `${activeCount} ${active === 'all' ? 'pricing rule' : option.label.toLowerCase() + ' rule'}${activeCount === 1 ? '' : 's'}` : `No ${option.label.toLowerCase()} rules`, subhead: `${averageAmount ? `Average ${averageAmount} in this filtered projection. ` : ''}Review platform fallbacks and facility prices. Changes remain unavailable until facility authority is proved.` };
  // selection excluded by decision: selected-facility mutation authority is
  // unproved, so read-only pricing evidence does not advertise bulk work.
  ...
  return <WorkspaceStage moduleRailItems={moduleRailItems} activePath="/pricing" routingPath={routingPath} onRailNavigate={onRailNavigate} rail={<PricingRail price={focusedPrice} loading={loading} hasFilter={hasFilter} />}>
    ...
    <ActivitySheet loading={loading} isFetching={isFetching} failedEmpty={failedEmpty} pagination={pagination} itemNoun="pricing rules" toolbar={<SheetToolbar searchValue={filters.search} onSearchCommit={(search) => setFilters((prev) => ({ ...prev, search }))} searchPlaceholder="Search pricing rules..." searchTestId="pricing-sheet-search" onRefresh={retry} refreshing={isFetching} refreshNoun="pricing rules" />} errorBanner={error && rows.length ? <div className="mt-3 rounded-inner bg-amber-500/10 px-4 py-3 text-sm text-amber-800 dark:text-amber-200">Pricing did not refresh. Showing the last loaded rules.</div> : null}>
      ...
      <div ref={listRef} tabIndex={0} onKeyDown={onKeyDown} aria-label="Pricing rules list" style={{ outline: 'none' }} className="mt-3 min-h-0 flex-1 overflow-y-auto rounded-card bg-background/30 p-3 no-scrollbar dark:bg-black/[0.08]">
        <div className={`grid ${GRID} items-center gap-2 px-4 pb-3 pt-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground`}><span>Rule</span><span>Family</span><span>Scope</span><SortableColumnHeader label="Updated" sortKey="updated_at" sortConfig={sortConfig} onSort={onSort} /><span className="justify-self-end">Action</span></div>
        ...
        {!loading && !error && totalCount === 0 && <EmptyState icon={BadgeDollarSign} heading={hasFilter ? 'No matching pricing rules' : 'No pricing rules'} body={hasFilter ? 'Change the family, scope, or search.' : 'Pricing rules for this scope will appear here.'} />}
        {!loading && rows.map((row) => <ListRowShell key={`${row.family || row._pricingType}-${row.id}`} id={row.id} dataAttrName="data-pricing-row" gridCols={GRID} selected={focusedPrice?.id === row.id} onFocus={() => setFocused(row.id)} onOpen={() => setFocused(row.id)}><div className="flex min-w-0 items-center gap-3"><span className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-icon ${scopeTone(row)}`}>{isGlobal(row) ? <Globe className="h-4 w-4" /> : <Building2 className="h-4 w-4" />}</span><div className="min-w-0"><div className="truncate text-[15px] font-semibold">{name(row)}</div><div className="mt-1 text-xs text-muted-foreground">{amount(row)}</div></div></div><span className="text-sm capitalize text-muted-foreground">{row.family || row._pricingType || 'service'}</span><StatusPill label={isGlobal(row) ? 'Platform' : 'Facility'} className={scopeTone(row)} compact /><span className="text-sm text-muted-foreground">{date(row.updated_at || row.created_at)}</span><Button variant="ghost" size="icon" onClick={(event) => { event.stopPropagation(); setFocused(row.id); }} className="h-8 w-8 justify-self-end rounded-pill bg-background/45 text-muted-foreground hover:bg-foreground hover:text-background" aria-label="Inspect pricing rule"><Info className="h-4 w-4" /></Button></ListRowShell>)}
      </div>
    </ActivitySheet>
  </WorkspaceStage>;
};
