...
import { AlertTriangle, Calendar, Clock, Crown, Eye, Info, Mail, MailX, UserPlus, Users } from 'lucide-react';
    ...
    >
      ...
      <div className="flex min-w-0 flex-wrap items-center gap-1.5">
        ...
        {isNewSubscriber(subscriber) && (
          <span className="rounded-pill bg-muted/40 px-2 py-0.5 text-[10px] font-semibold text-muted-foreground">New</span>
        )}
      </div>
      <span className="text-sm text-muted-foreground">{getJoinedDateLabel(subscriber)}</span>
      <Button
        variant="ghost"
        size="icon"
        onClick={(event) => {
