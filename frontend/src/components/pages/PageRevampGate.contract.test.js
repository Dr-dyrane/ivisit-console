...
describe('Today/Requests revamp gate contract', () => {
  ...
  it('keeps the user-requested Today and Requests canonical audit explicit', () => {
    ...
    expect(gate).toContain('Subscriptions is Page 17 intake audit only. It is not admitted and no visual revamp, subscriber edit/delete/status/type, welcome/custom/bulk email send, export, shared Requests pattern reuse, route-owned action promotion, or hardgate promotion is authorized until subscriber projection, email lifecycle, hidden shell acquisition, paid/billing semantics, and delivery-consequence blockers are closed.');
