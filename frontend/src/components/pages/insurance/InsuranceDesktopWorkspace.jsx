...
import { AlertTriangle, CheckCircle, Clock, Eye, Info, Percent, Shield } from 'lucide-react';
...
const IMPORTANCE = { all: 0, pending: 1, unverified: 2, active: 3, expired: 4 };
const GRID = 'grid-cols-[minmax(210px,1.8fr)_minmax(120px,1fr)_minmax(100px,auto)_minmax(105px,auto)_96px]';
const SELECTABLE_GRID = 'grid-cols-[28px_minmax(210px,1.8fr)_minmax(120px,1fr)_minmax(100px,auto)_minmax(105px,auto)_96px]';
...
const policyPill = (status) => resolveVital('insurance', normalizedStatus(status))?.pill || { label: 'Unknown', className: 'bg-muted/40 text-muted-foreground' };
const hasFilterValue = (value) => Array.isArray(value)
  ? value.length > 0
...
const count = (stats, rows, id) => {
  ...
  return rows.filter((row) => normalizedStatus(row.status) === id).length;
};
const date = (value) => value && !Number.isNaN(new Date(value).getTime()) ? new Date(value).toLocaleDateString() : 'Not set';
const percentage = (value) => value !== null && value !== undefined && value !== '' && Number.isFinite(Number(value)) ? `${Number(value).toLocaleString()}%` : 'Not set';

export const InsuranceDesktopWorkspace = ({ rows, stats, denied, loading, isFetching, error, filters, setFilters, filterSheetOpen, openFilters, retry, clearFilters, pagination, sortConfig, onSort, focusedPolicy, setFocused, onView, selectable = false, selectedIds = [], onToggleSelect, onSelectClick, onSelectAll, allSelected = false, someSelected = false, moduleRailItems, routingPath, onRailNavigate }) => {
  ...
  // Selection is an admin-only review affordance. The page owns the visible-row selection
  // state and keeps the accompanying bulk command disabled until write authority is proved.
  ...
  const signalHeadline = activeCount
    ...
    : active === 'all' ? 'No policies' : `No ${activeOption.label.toLowerCase()} policies`;
  const signal = denied
    ? { icon: Shield, tone: 'muted', label: 'Access unavailable', headline: 'Insurance access is unavailable', subhead: 'This account does not have access to policy records.' }
    : failedEmpty
    ? { icon: AlertTriangle, tone: 'danger', label: 'Load failed', headline: 'Insurance did not load', subhead: 'Try again to view policy records.' }
    : { icon: activeOption.icon, tone: activeCount ? (active === 'active' ? 'clear' : active === 'expired' ? 'warning' : 'primary') : 'muted', label: activeOption.label, headline: signalHeadline, subhead: 'View policy details and billing outcomes. Policy changes are currently unavailable.' };
  ...
  const hasFilter = Boolean(filters.search || active !== 'all' || hasFilterValue(filters.status) || hasFilterValue(filters.type) || hasFilterValue(filters.verified) || filters.created_at?.start || filters.created_at?.end);

  return <WorkspaceStage moduleRailItems={moduleRailItems} activePath="/insurance" routingPath={routingPath} onRailNavigate={onRailNavigate} rail={<InsuranceRail policy={focusedPolicy} denied={denied} loading={loading} hasFilter={hasFilter} onView={onView} />}>
    <SignalPanel signal={signal} loading={loading} toneClassMap={TONES}>
      <KpiStrip options={OPTIONS} getCount={(id) => count(stats, rows, id)} kpiFilter={active} setKpiFilter={(id) => setFilters((prev) => ({ ...prev, kpiFilter: id }))} loading={loading} isFetching={isFetching} pinnedIds={['pending', 'unverified']} importance={IMPORTANCE} defaultId="all" dataAttr="data-insurance-state" />
    </SignalPanel>
    <ActivitySheet loading={loading} isFetching={isFetching} failedEmpty={failedEmpty} pagination={pagination} itemNoun="policies" toolbar={<SheetToolbar searchValue={filters.search} onSearchCommit={(search) => setFilters((prev) => ({ ...prev, search }))} searchPlaceholder="Search policy, provider, or plan..." searchTestId="insurance-sheet-search" onRefresh={retry} refreshing={isFetching} refreshNoun="policies" onOpenFilters={openFilters} filterSheetOpen={filterSheetOpen} filtersActive={hasFilter} />} errorBanner={!denied && error && rows.length ? <div className="mt-3 rounded-inner bg-amber-500/10 px-4 py-3 text-sm text-amber-800 dark:text-amber-200">Insurance did not refresh. Showing the previous results.</div> : null}>
      <div ref={listRef} tabIndex={0} onKeyDown={onKeyDown} aria-label="Insurance policies list" style={{ outline: 'none' }} className="mt-3 min-h-0 flex-1 overflow-y-auto rounded-card bg-background/30 p-3 no-scrollbar dark:bg-black/[0.08]" data-testid="insurance-list">
        <div className={`grid ${selectable ? SELECTABLE_GRID : GRID} items-center gap-2 px-4 pb-3 pt-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground`}>
          {selectable && (
            <Checkbox
        ...
        {!loading && denied && <EmptyState icon={Shield} heading="Insurance access unavailable" body="This account does not have access to policy records." />}
        {!loading && !denied && failedEmpty && <LoadErrorState title="Insurance did not load" message={error} onRetry={retry} />}
        {!loading && !denied && !error && pagination.totalCount === 0 && <EmptyState icon={Shield} heading={hasFilter ? 'No matching policies' : 'No policies'} body={hasFilter ? 'Change filters or search again.' : 'Policy records will appear here.'}>{hasFilter && <Button variant="ghost" onClick={clearFilters} className="rounded-pill bg-muted/30 px-5 font-semibold active:scale-95">Show all policies</Button>}</EmptyState>}
        {!loading && !denied && rows.map((policy) => {
          const pill = policyPill(policy.status);
          const policyLabel = policy.policy_holder_name || policy.policy_number || 'Insurance policy';
          const checked = selectedIds.includes(policy.id);
          return <ListRowShell key={policy.id} id={policy.id} dataAttrName="data-insurance-row" gridCols={selectable ? SELECTABLE_GRID : GRID} selected={focusedPolicy?.id === policy.id} onFocus={() => setFocused(policy.id)} onOpen={() => onView(policy)}>
            {selectable && (
              <Checkbox
                checked={checked}
...
const InsuranceRail = ({ policy, denied, loading, hasFilter, onView }) => {
