...
describe('AmbulancesPage visual-start repair contract', () => {
  ...
  it('records fleet route-data and visual-start repair while full admission remains blocked', () => {
    ...
    expect(queryHookSource()).toContain('queryFn: ({ signal }) => getAmbulancesPageData({ ...filter, abortSignal: signal })');
    expect(queryHookSource()).toContain('retry: shouldRetryAmbulanceQuery');
    ...
    expect(modal).toContain('filterAmbulanceStationOptions');
    expect(modal).toContain('assertAmbulanceWriteScope(payload, actorScope)');
    expect(modal).toContain('stationOutOfScope');
    expect(modal).toContain('Select an authorized station');
    expect(modal).toContain("if (isCreate) payload.status = formData.status || 'available';");
    ...
    expect(service).toContain('export function filterAmbulanceStationOptions');
    expect(service).toContain('export function assertAmbulanceWriteScope');
    expect(service).toContain("error.code = 'AMBULANCE_SCOPE_DENIED';");
    expect(service).toContain('assertAmbulanceWriteScope(scopedInput, actor);');
    expect(service).toContain('assertAmbulanceWriteScope(input, actor);');
    expect(service).not.toContain("'call_sign', 'type', 'status', 'vehicle_number'");
    expect(service).toContain('export async function getAmbulancesPageData');
    expect(service).toContain('applyQueryAbortSignal(countQuery, request.signal)');
    expect(service).toContain('getAmbulancePageStats(user, statsFilters, request.signal)');
    expect(service).toContain('enrichAmbulanceRowsForPage(pageResult.data || [], request.signal)');
    expect(service).not.toContain('withTimeout(dataQuery');
