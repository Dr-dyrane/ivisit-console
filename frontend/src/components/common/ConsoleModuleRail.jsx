...
export const ConsoleModuleRail = ({
  ...
  <nav className="hidden lg:flex absolute left-5 top-1/2 z-20 -translate-y-1/2 flex-col gap-3 rounded-full bg-card/70 p-2 shadow-[0_18px_50px_rgb(0_0_0/0.10)] backdrop-blur-2xl dark:bg-white/[0.06]">
    {items.map(({ id, icon: Icon, label, path }) => {
      ...
      return (
        <button
          ...
          className={`h-[38px] w-[38px] rounded-full flex items-center justify-center transition-[background,color,transform] duration-200 focus-visible:bg-primary/15 focus-visible:text-foreground active:scale-[0.96] ${
            isActive
