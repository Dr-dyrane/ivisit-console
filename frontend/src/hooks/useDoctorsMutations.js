...
import { useInvalidateDoctors } from './useDoctorsQuery';

/**
 * useDoctorsMutations - the REFERENCE optimistic-mutation pattern for the console
 * (Console Layer-Model Plan step S2-4, CONSOLE_LAYER_MODEL_PLAN.md:195). It proves
 * the L2+L3 write contract on the one domain already on React Query (useDoctorsQuery)
 * before any other domain copies it, so keep this file clean and documented - other
 * domains are expected to duplicate its shape.
 *
...
// Mirrors useDoctorsQuery.js:20 (`queryKey: ['doctors', filter]`). The root
// ['doctors'] is the invalidation/cancel prefix - the exact same prefix that
// useInvalidateDoctors (useDoctorsQuery.js:42-45) already uses, so read,
// mutation-settlement, and realtime-invalidate all agree on one key family.
export const DOCTORS_KEY_ROOT = ['doctors'];

/**
 * Build the exact list key a given filter is cached under. Passing the same
 * `filter` the page passed to useDoctorsQuery targets that page's cache entry;
 * the default {} matches the unfiltered list.
 */
export const doctorsListKey = (filter = {}) => ['doctors', filter];

// --- Pure optimistic reducers --------------------------------------------
// useDoctorsQuery caches the shape { data: Doctor[], count, stats } under the
// list key (useDoctorsQuery.js:28-30). These pure helpers transform that shape
// so an optimistic write never has to reach into React Query internals. They
// are exported so callers (and the reference tests) can reuse them.

const emptyDoctorsCache = () => ({ data: [], count: 0, stats: undefined });

/**
 * Insert a new doctor, or merge onto an existing one (matched by id), at the
 * front of the cached list. Count grows only for a genuine insert.
 */
export function applyOptimisticUpsert(cache, doctor) {
  const base = cache && Array.isArray(cache.data) ? cache : emptyDoctorsCache();
  const id = doctor ? doctor.id : undefined;
  const exists = id != null && base.data.some((row) => row.id === id);
  const data = exists
    ? base.data.map((row) => (row.id === id ? { ...row, ...doctor } : row))
    : [{ ...doctor }, ...base.data];
  return {
    ...base,
    data,
...
/**
 * Remove a doctor by id from the cached list. Count shrinks only if the row
 * was actually present, and never goes below zero.
 */
export function applyOptimisticRemove(cache, doctorId) {
  if (!cache || !Array.isArray(cache.data)) return cache;
  const existed = cache.data.some((row) => row.id === doctorId);
  return {
    ...cache,
    data: cache.data.filter((row) => row.id !== doctorId),
 ...
 * QueryClient in unit tests (@testing-library/react's renderHook is not a
 ...
 */
export function buildDoctorsMutationOptions({
  queryClient,
  mutationFn,
  applyOptimistic,
  invalidate,
  ...
  return {
    ...
    async onMutate(variables) {
      // Stop in-flight doctors refetches so they cannot overwrite the
      // optimistic value after we set it.
      await queryClient.cancelQueries({ queryKey: DOCTORS_KEY_ROOT });
...
/**
 * useDoctorsMutations - React Query mutation wired with the optimistic pattern.
 *
 * @param {object} params
 * @param {(variables:any)=>Promise<any>} params.mutationFn - e.g.
 *   createDoctor / updateDoctor / deleteDoctor from doctorsService.
...
export default useDoctorsMutations;
