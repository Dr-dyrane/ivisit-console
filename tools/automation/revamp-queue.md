...
| Insurance (Page 12) NON-PAGE surface -> borderless canon | surface-revamp-agent | `frontend/src/components/context/InsurancePanel.jsx`, `frontend/src/components/mobile/MobileInsurance.jsx`, `frontend/src/components/modals/InsuranceModal.jsx` | in_progress 2026-07-07: chrome-only canon on the three Insurance non-page surfaces (panel/mobile/modal). Squircle-lg->card, geo-round->icon/inner, rounded-full/2xl/3xl/xl->pill/card/inner, shadcn Badges->rounded-pill spans, bg-info/20 dot->bg-muted/40, remove shadow-premium/border-t/border-0. DISJOINT from controller's `InsuranceManagementPage.jsx` page-file claim (line 100). No form/data/handler change. |
