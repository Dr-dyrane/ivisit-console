...
| Map/GodMode cluster -> borderless canon (chrome-only) | canon-cleanup-agent | `frontend/src/components/pages/GodModeMap.jsx`, `frontend/src/components/mobile/MobileMap.jsx`, `frontend/src/components/context/MapPanel.jsx` + hardgate on `frontend/src/components/map/{LiveStatsPanel,RecentAlertsPanel,MarkerDetailPanel,MapLayerControls}.jsx` | in_progress 2026-07-07: chrome-only strict-radius canon. All target/map files verified to have ZERO real git diff at start (the codex-dirty flags on GodModeMap/MobileMap/MarkerDetailPanel/MapLayerControls are pure EOL normalization — `git diff` yields no hunks), so this lane is collision-safe. Converting non-canonical radius (rounded-2xl/3xl/xl/lg/full/[Npx] + squircle-sm/lg/xl) to canonical tokens, removing border-0/border/ring/focus:outline-none, replacing shadow-premium/shadow-2xl with soft/canonical shadow, neutralizing red theme tokens (primary/info/success/warning -> literal sky/emerald/amber/rose or neutral) incl. inline map style hsls, shadcn Badge->rounded-pill span. Preserving all map logic/markers/providers/handlers. |
